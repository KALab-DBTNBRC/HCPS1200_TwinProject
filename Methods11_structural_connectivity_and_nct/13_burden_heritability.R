# 13_burden_heritability.R
# ------------------------------------------------------------------------
# Heritability of a single burden-concentration summary phenotype per
# substrate (MIND, SC) -- the participation ratio of each subject's
# cross-target mean burden map. Split out from the combined energy+
# burden lean script for clarity. 
#
# ACE model, phenotype residualisation, and LRT structure copied exactly
# from the recovered nct_q45_heritability.R (same convention as
# 12_energy_heritability.R -- kept consistent deliberately).
#
#   {PROJECT_ROOT}/control_burden/burden_PR_summary_lean.csv
#   columns: subj_id, MIND_BurdenPR, SC_BurdenPR
# That export is a ~10-line addition to 11_control_burden.py's existing
# "control-role" section:
#   role = mean([burden_store[(sub, net)] for net in NETWORK_ORDER], axis=0)  # (N, 360)
#   pr   = participation_ratio(role)  # (N,) -- same formula already used there
# Add that export before running this script.
# ------------------------------------------------------------------------
...
BURDEN_DIR <- file.path(CTRL_DIR, "control_burden")
...
cat(sprintf("[%s] Control burden heritability (summary phenotypes)\n", format(Sys.time(), "%H:%M:%S")))


# 1. LOAD PHENOTYPES (requires the CSV export documented above)
burden_pr_path <- file.path(BURDEN_DIR, "burden_PR_summary_lean.csv")
if (!file.exists(burden_pr_path)) {
  stop(sprintf(
    "Missing %s -- export it from 11_control_burden.py's control-role section first (see this script's header comment for the exact ~10-line addition needed).",
    burden_pr_path))
}
pheno <- read.csv(burden_pr_path, stringsAsFactors = FALSE)
burden_cols <- setdiff(colnames(pheno), "subj_id")
cat(sprintf("  Phenotypes: %d (%s)\n", length(burden_cols), paste(burden_cols, collapse = ", ")))
...
# 4. ACE MODEL -- identical to 12_energy_heritability.R
...
# 5. RUN
cat(sprintf("[%s] Running ACE for %d burden phenotypes...\n", format(Sys.time(), "%H:%M:%S"), length(burden_cols)))
results <- list()
...
out_path <- file.path(OUT_DIR, "nct_burden_heritability.csv")
